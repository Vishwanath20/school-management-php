...
// Fetch all resources with their master and sub titles
$stmt = $pdo->query("
    SELECT r.*, m.title as master_title, s.title as sub_title 
    FROM free_resources r
    JOIN free_resource_master m ON r.master_id = m.id
    JOIN free_resource_master_sub s ON r.sub_id = s.id
    ORDER BY m.display_order ASC, s.title ASC, r.title ASC
");
        ...
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-header" style="display:flex;justify-content:space-between;">
                    <h4 class="card-title">Free Resources</h4>
                    <div>
                        <a href="manage.php" class="btn btn-primary btn-icon-text">
                            <i class="mdi mdi-plus btn-icon-prepend"></i>Add Resource</a>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($resource['sub_title']); ?></td>
...
$(document).ready(function() {
    ...
    $('.status-toggle').on('change', function() {
        ...
        $.ajax({
            url: '../../api/freeresource/update-status.php',
            type: 'POST',
    ...
    $('.delete-resource').on('click', function() {
        ...
        if (confirm('Are you sure you want to delete this resource?')) {
            $.ajax({
                url: '../../api/freeresource/delete.php',
                type: 'POST',
                data: { id: resourceId },
                success: function(response) {
                    if (response.status === 'success') {
